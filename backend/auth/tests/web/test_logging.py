...
* ``RequestContextMiddleware`` logs exactly one ``request`` line per HTTP call
  (method / path / status / duration), echoes an inbound ``X-Request-ID`` and
  mints one otherwise, and never leaks it across requests.
"""
...
async def test_inbound_request_id_is_echoed(client):
    ac, _, _ = client
    resp = await ac.get("/health", headers={"X-Request-ID": "req-42"})
