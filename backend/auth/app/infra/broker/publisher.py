...
# Fixed wire fields for the registration-verification email. The notifications
# service routes on the forced `email` (transactional channel), so `type`/`title`
# are for its history/display only.
_VERIFY_TYPE = "email_verification"
_VERIFY_TITLE = "Verify your email"


class RabbitMQPublisher(NotificationPublisher):
    """Publishes a ``NotificationEvent``-shaped message to the notifications broker.
    ...
    async def publish_verification(
        ...
        # manual resend); pool it only if it ever gets hot.
